...
decltype(auto) test_far(uint32_t* target, uint32_t const* callback) {
    constexpr size_t hookSize = 32;
    constexpr size_t trampolineSize = 64;
...
void perform_far_hook_test(uint8_t* to_hook) {
    printf("TO HOOK: %p\n", to_hook);
    print_decode_loop(reinterpret_cast<uint32_t*>(to_hook), 6);
    puts("TEST FAR...");
...
void test_far_bls_tbzs_within_hook() {
...
void test_near_ldr_ldrb_tbnz_bl() {
...
void test_far_ldr_ldrb_tbnz_bl() {
...
int main() {
    ...
    test_near_ldr_ldrb_tbnz_bl();
    test_far_no_fixups();
    test_far_bls_tbzs_within_hook();
    test_far_ldr_ldrb_tbnz_bl();
}
