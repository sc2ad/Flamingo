...
Trampoline TrampolineAllocator::Allocate(std::size_t trampolineSize) {
  ...
  for (auto& page : pages) {
    ...
    if (PageSize - page.used_size > trampolineSize) {
      // If we can fit in our existing page, we should try to
      // Mark full page as rxw AGAIN (since we may have swapped it off before)
      if (::mprotect(page.ptr, PageSize, PROT_READ | PROT_WRITE | PROT_EXEC) != 0) {
        // Log error on mprotect!
        FLAMINGO_ABORT("Failed to mark allocated page at: {} as +rwx. err: {}", fmt::ptr(page.ptr), std::strerror(errno));
      }
      // We have to be aligned 16 here
      constexpr static auto kAlignment = 0x10U;
      auto unaligned_start = reinterpret_cast<uintptr_t>(page.ptr) + page.used_size;
      uintptr_t aligned_start;
      if (unaligned_start % kAlignment != 0) {
        // Pad upwards
        auto pad_bytes = (kAlignment - (unaligned_start % kAlignment));
        aligned_start = unaligned_start + pad_bytes;
        page.used_size += pad_bytes;
      } else {
        aligned_start = unaligned_start;
      }
      Trampoline to_ret(reinterpret_cast<uint32_t*>(aligned_start), trampolineSize / sizeof(uint32_t), page.used_size);
  ...
  if (::posix_memalign(&ptr, PageSize, PageSize) != 0) {
    ...
    FLAMINGO_ABORT("Failed to allocate trampoline page of size: {} for size: {}. err: {}", PageSize, trampolineSize, std::strerror(errno));
  }
  ...
  // Convert from trampoline size to number of instructions in the trampoline
  return { static_cast<uint32_t*>(ptr), trampolineSize / sizeof(uint32_t), page.used_size };
}
